...
class SpeechTranscriptionApp:
    ...
    def synth(self):
        agi.appexec('MRCPSynth', "\\\"%s\\\",l=%s&v=%s&p=2" % (self.prompt,self.syntlanguage['Rcode'],self.syntlanguage['Scode']))
    ...
    def compose_speech_options1(self):
        ...
        if self.action=="Starttalking":
            options = 'plt=1&b=1&sct=1000&sint=15000&nit=10000&p=2'
        else:
            options = 'plt=1&dit=1000&b=1&sct=600&sint=1000&nit=3000&p=2'
    ...
    def get_brandprompt(self):
        prompt_info = toba_db.get_prompt_info("Selectlanguage_dtmf")
        if prompt_info['status'] == True: 
            self.brandprompt= self.get_translation(prompt_info['PROMPTTEXT'],str(self.language['Tcode']),False)
        else:
    ...
    def get_prompt(self,action):
        ...
        prompt_info = toba_db.get_prompt_info(action)

        if prompt_info['status'] == True: 
            prompt=prompt_info['PROMPTTEXT']
            dtmfprompt=toba_db.get_prompt_info('dtmf_prompt')
            ...
            self.prompt=self.get_translation(prompt,str(self.language['Tcode']),False)
    ...
    def get_caller_info(self):
        ...
        caller_info= toba_db.get_caller_info(self.get_callerid())
    ...
    def get_language_info_from_db(self,language):
        """get language data from tobadb"""
        result=toba_db.get_language_info(language)
    ...
    def get_translation(self,text,Tcode,validate):
        ...
        detectedLanguage=results["detectedSourceLanguage"]
        if results["detectedSourceLanguage"]=='zh-CN':
            detectedLanguage='zh-TW'
        if result and detectedLanguage==Tcode and self.action=='Starttalking' and validate==True:
            if Tcode==self.language['Tcode']:
                Tcode=self.guest_language['Tcode']
                self.syntlanguage=self.guest_language
            elif Tcode==self.guest_language['Tcode']:
                Tcode=self.language['Tcode']
                self.syntlanguage=self.language
            result=self.get_translation(str(result),Tcode,False)
    ...
    def translate_prompt_earlier(self):
        ...
        if self.get_language_code()==guest_language['Rcode'].lower() and self.get_language_code()!=self.language:
            ...
            prompt=self.get_translation(str(transcribed),primary_language['Tcode'],True)
        ...
        if self.get_language_code()==primary_language['Rcode'].lower() and self.get_language_code()!=self.language:
            ...
            prompt=self.get_translation(str(transcribed),guest_language['Tcode'],True) 
    ...
    def add_db_record(self):
        ...
        result = toba_db.add_record(self.starttime, Duration, Status,Name,id,CLi,_From,Call_ID,User_Agent,User_language,Guest_Language_Code,server)
        if result['status'] == True:
            agi.verbose('database %s' % result['string'])
        else:
            agi.verbose('database %s' % result['error_cause'])
